...
async function loadDashboard(){
  const sources=['data/dashboard.json','data/mock-data.json'];
  let d=null;
  ...
  if(!d) throw new Error('No dashboard data found');
  ...
  render();
  [...new Set(DATA.sponsorships.map(x=>x.type).filter(Boolean))].sort().forEach(t=>$('#type').insertAdjacentHTML('beforeend',`<option>${t}</option>`));
}

loadDashboard().catch(err=>{
  console.error(err);
  $('#kpis').innerHTML='<p>Dashboard data not found. Check data/dashboard.json</p>';
});
/*
